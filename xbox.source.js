    ...
    var xBox = new AP.Class({
        ...
        */
        ...
        _iframeOnLoad: function(e, self) {
            ...
            //为iframe添加关闭按钮
            try{
                var iframeWin = D.get(this.cfg.IFRAME).contentWindow;
	            iframeWin.E && iframeWin.E.on(iframeWin.D.query(".xbox-close-link", iframeWin.document), "click", function(e){
	    	    	AP.widget.xBox.hide();
	    	    	E.preventDefault(e);
	    	    });
    	    }catch(e){}
        ...
        _getIFRHeight: function() {
			if(this.options.cross_domain && this.options.height){
    ...
    function hidexBox () {
		...
		try{
			if (AP.cache.xbox.onHideEvent){ 
				...
				var onHideEventCache = AP.cache.xbox.onHideEvent;
			}
		}catch(e){}

    	//delete global cache
    	AP.cache.xbox = {};
		//不清空onHideEvent事件 add by guangao
		AP.cache.xbox.onHideEvent = onHideEventCache;
    }
